...
# Copyright 2020 Alvaro Correia.
# Distributed under the MIT Software License.
# (See accompanying file MIT-LICENSE or https://opensource.org/license/mit/)
...
def get_data(name, max_rows = 10000000):
    if 'wine' in name:
        ...
        data = data.drop(columns=['region.centroid.col', 'region.pixel.count'])
        cols = ['short.line.density.5', 'short.line.density.2', 'class']
        cols1 = ['region.centroid.row', 'vedge.mean', 'vegde.sd', 'hedge.mean', 'hedge.sd',
                 'intensity.mean', 'rawred.mean', 'rawblue.mean', 'rawgreen.mean', 'exred.mean', 'exblue.mean',
                 'exgreen.mean', 'value.mean', 'saturation.mean', 'hue.mean']
        data.loc[:, cols] = get_dummies(data[cols])
        data = data[:max_rows]
        ncat = learncats(data.values, classcol=-1, continuous_ids=[data.columns.get_loc(c1) for c1 in cols1])
        data = data.values.astype(float)
    elif 'german' in name:
        data = pd.read_csv('./data/german.csv', sep=' ', header=None)
        cols2 = [0, 2, 3, 5, 6, 8, 9, 11, 13, 14, 16, 18, 19, 20]
        cols3 = [1, 4, 7, 10, 12, 15, 17]
        data.iloc[:, cols2] = get_dummies(data[cols2])
        data = data[:max_rows]
        ncat = learncats(data.values, classcol=-1, continuous_ids=cols3)
        data = data.values.astype(float)
        ...
        cols8 = ['Speaker_Number', 'Sex', 'Class']
        data.loc[:, cols8] = get_dummies(data[cols8])
        data = data[:max_rows]
        ncat = learncats(data.values, classcol=data.shape[1] - 1)
        data = data.values.astype(float)
        ...
        cols4 = ['Wifes_education', 'Husbands_education', 'Wifes_religion', 'Wifes_now_working%3F',
                 'Husbands_occupation', 'Standard-of-living_index', 'Media_exposure', 'Contraceptive_method_used']
        cols5 = ['Wifes_age', 'Number_of_children_ever_born']
        data.loc[:, cols4] = get_dummies(data[cols4])
        data = data[:max_rows]
        ncat = learncats(data.values, classcol=data.shape[1] - 1)
        data = data.values.astype(float)
        ...
        cols6 = ['day', 'class']
        cols7 = ['date', 'period', 'nswprice', 'nswdemand', 'vicprice',
                 'vicdemand', 'transfer']
        data.loc[:, cols6] = get_dummies(data[cols6])
        data = data[:max_rows]
        ncat = learncats(data.values, classcol=-1, continuous_ids=[data.columns.get_loc(c2) for c2 in cols7])
        data = data.values.astype(float)
