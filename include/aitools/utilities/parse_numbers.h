...
//
/// \file aitools/parse_numbers.h
/// \brief add your file description here.

#ifndef AITOOLS_PARSE_NUMBERS_H
#define AITOOLS_PARSE_NUMBERS_H
...
std::vector<double> parse_double_sequence(const std::string& text)
{
  ...
  for (double f = std::strtold(p, &end); p != end; f = std::strtold(p, &end))
  {
    ...
    if (errno == ERANGE)
    {
      AITOOLS_LOG(log::debug) << "WARNING: parse_double_sequence got a range error in ";
      errno = 0;
    }
    result.push_back(f);
  }
