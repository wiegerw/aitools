...
//
/// \file aitools/logger.h

#ifndef AITOOLS_LOGGER_H
#define AITOOLS_LOGGER_H
...
class output_policy
{
  public:
    ...
    virtual void output(log_level_t level, const std::string& hint, time_t timestamp, const std::string& msg, bool print_time_information) = 0;
};
...
class logger
{
  ...
  public:
    ...
    {
      auto i = hint_to_level().find(default_hint());
    ...
    {
      auto i = output_policies().find(&policy);
    ...
    {
      auto i = hint_to_level().find(hint);
      if (i != hint_to_level().end())
      {
        return i->second;
      }
...
public:
  ...
  /// - log level
  /// - indentation
  static std::string format(log_level_t level, const std::string& hint, time_t timestamp, const std::string& msg, bool print_time_information);
};
...
class file_output: public output_policy
{
  protected:
    ...
    static FILE* get_stream(const std::string& hint)
    {
      auto i = hint_to_stream().find(hint);
  ...
  public:
    ...
    /// atomic.
    void output(const log_level_t level, const std::string& hint, const time_t timestamp, const std::string& msg, const bool print_time_information) override
    {
...
typedef logger aitools_logger;

/// \brief Unless otherwise specified, we compile away all debug messages that have
/// a log level greater than AITOOLSMaxLogLevel.
constexpr log_level_t AITOOLSMaxLogLevel = aitools::log::debug;

/// \returns True whenever the logging for the given level is enabled.
constexpr bool AITOOLS_LOG_ENABLED(const log_level_t level)
{
  return level <= AITOOLSMaxLogLevel && level <= aitools_logger::default_reporting_level();
}

/// \returns True whenever the logging for the given level and hint is enabled.
constexpr bool AITOOLS_LOG_ENABLED(const log_level_t level, const std::string& hint)
{
  return level <= AITOOLSMaxLogLevel && level <= aitools_logger::get_reporting_level(hint);
}

} // namespace aitools::log

/// \brief AITOOLS_LOG(level) or AITOOLS_LOG(level, hint) provide the stream used to log.
/// \details Uses variadic macros to allow AITOOLS_LOG(level) as well as AITOOLS_LOG(level, "hint").
#define AITOOLS_LOG(...) if (aitools::log::AITOOLS_LOG_ENABLED(__VA_ARGS__)) aitools::log::aitools_logger().get(__VA_ARGS__)

#endif // AITOOLS_LOGGER_H
